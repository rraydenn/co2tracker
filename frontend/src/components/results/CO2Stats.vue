  ...
  <div class="co2-stats">
    ...
    <div v-else class="stats-container">
      ...
      <div class="eco-tips">
        ...
        <ul>
          <li>
            Privilégiez les transports en commun pour vos trajets quotidiens
          </li>
...
export default defineComponent({
  ...
  setup(_, { expose }) {
    ...
    const stats = computed<TripStats>(() => {
      log(
        `CO2Stats - Computing statistics from ${trips.value.length} trips`,
        "debug"
      );
      if (trips.value.length === 0) {
        ...
        return {
          ...
          ranking: ranking.value?.[0]?.rank
            ? `${ranking.value[0].rank}`
            : "N/A",
        };
      }

      // Calculate total CO2
      const totalCO2 = trips.value.reduce(
        (sum, trip) => sum + trip.co2Total,
        0
      );
      log(`CO2Stats - Total CO2 calculated: ${totalCO2} kg`, "debug");

      // Calculate total distance
      const totalDistance = trips.value.reduce(
      ...
      trips.value.forEach((trip) => {
        const transportName =
          trip.transport?.name || `Transport ${trip.transportId}`;
        transportCounts[transportName] =
          (transportCounts[transportName] || 0) + 1;
      });
      ...
      log(
        `CO2Stats - Most used transport: ${mostUsedTransport} with count: ${maxCount}`,
        "debug"
      );
    ...
    const fetchTrips = async () => {
      ...
      try {
        log(
          "CO2Stats - Making API request with token:",
          "debug",
        ...
        log(
          `CO2Stats - Received ${data.length} trips for stats calculation`,
          "debug"
        );
        ...
        console.error(
          "### Debug: CO2Stats - Failed to fetch trips for stats:",
          err
        );
        error.value =
    ...
    const fetchRanking = async () => {
      ...
      try {
        ...
        log(
          "CO2Stats - Ranking API response status:",
          "debug",
          response.status
        );
    ...
    // Expose reloadStats method for parent components
    expose({ reloadStats: loadStats });
